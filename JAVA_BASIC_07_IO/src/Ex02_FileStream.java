...
I/O 자원은 개발자 직접적으로 자원에 해제 (워드 파일 작업중에 같은 파일 열었어요...상황 기조파일 닫기)
>> Close()
>> 예외처리 (강제된) 사용하는 I/O 클래스는 강제로 예외처리 요구하는 코드가 많다
...
public class Ex02_FileStream {
	...
	public static void main(String[] args) {
		...
		/*
		 FileOutputStream
		 1. write  파일이 존재하지 않으면 >> 자동 파일 생성9ㅊㄱㄷㅁㅅㄷ 가능)
		...
		int data=0;
		while((data=fs.read())!=-1) { //"C:\\Temp\\new.txt"
			//System.out.println(" 정수: "+ data + ":" + (char)data);
			fos.write(data); //출력 스트림 // C:\\Temp\\new.txt
		}
		}catch(Exception e) {
			e.printStackTrace();
