...
여러명이 탈 수 있는 버스를 대영해서 한번 보내고 처리...

버스: Buffer
1. I/O 성능 개선(접근 횟수)
2. Line 단위(엔터)

 BufferedOutputTream(보조 스트림) >> 독자적인 작업 불가 >> File(input, output)Stream
 **/
public class Ex04_Stream_Buffer {
	...
	public static void main(String[] args) {
		...
		try {
			...
			/*
			 JAVA Buffer (8k, 8192 byte)
			 1. Buffer 안에 내용이 채워지면 스스로 출발(버퍼를 비우는 작업)
			 2. 강제로 출발 (buffer) 강제 비우기 : flush() or close()히면 자동 flush()
			 3. 게발 close() 자원해제 >> 내부적으로 flush()
			 */
		}catch(Exception e) {
